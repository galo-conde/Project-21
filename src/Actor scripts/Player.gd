...
func _physics_process(delta):
	match state: #enum which is close to a switch function
		MOVE:
...
func move_state(delta): #this function is for movement
	...
	#walking sideways is faster, so normalized() sets up the vector to a circle instead of radius 1
